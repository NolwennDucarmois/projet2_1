...
public class ModelCourseDBHyb extends DAOCourse {

    protected Connection dbConnect;

    public ModelCourseDBHyb() {
    ...
    public List<Pays> listePaysPilotes(Course course) {
        ...
        String query = "SELECT * from vuelistePaysPilotes WHERE idCourse = ?";
        try (PreparedStatement pstm = dbConnect.prepareStatement(query)) {
            pstm.setInt(1, course.getIdCourse());
            ResultSet rs = pstm.executeQuery();
            while (rs.next()) {
    ...
    public List<Pilote> listePilotesDuPays(Course course) {
        ...
        String query = "select * from vuelistePilotesDuPays WHERE idCourse=?";
        try (PreparedStatement pstm = dbConnect.prepareStatement(query)) {
            pstm.setInt(1, course.getIdCourse());
            ResultSet rs = pstm.executeQuery();
            while (rs.next()) {
    ...
    public List<Pilote> getPilotesCourse(Course course) {
        List<Pilote> liste = new ArrayList<>();
        String query = "select * from getPilotesCourse where idcourse = ?";
        try (PreparedStatement pstm = dbConnect.prepareStatement(query)) {
            pstm.setInt(1, course.getIdCourse());
            ResultSet rs = pstm.executeQuery();
            while (rs.next()) {
